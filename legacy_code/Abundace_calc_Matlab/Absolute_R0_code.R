...
g_area = nc_open("/Users/pratik/Desktop/R0_computations/Input_files/R0_files/GFED_grid_025.nc")      #for 0.25 resolution 

area_grid<-ncvar_get(g_area,"GFED4.1s_1997.hdf5:1")
#g_area<-nc_open("/Users/pratik/Desktop/R0_computations/Input_files/R0_files/gridarea.nc")    # for 0.5 resolution
...
month_yr<-expand.grid(month=1:12,year= 1995:2019) %>%   # for only one year from 1st month to 12th month and year you want
  mutate(n=1:n())
...
## go through each year population file, and repeat each year population 12 times and put it an empty matrix with rows same all latlong combination to match the format of each matrix 

#setwd("/Users/pratik/Desktop/ae_albopictus_model")
setwd("/Users/pratik/Desktop/Collab_data/Zia_collab/Pop_count_data_USA")  # for EU

temp = nc_open("pop_count_1996_USA_0.25.nc")  # A nc file is required of same dimension to set rows of matrix, any input file will be ok, for only once
...
pop_month <- matrix(nrow = length(df$x), ncol = 0)    # instead of this we can also swap the first two columns of output of function


# path and file name, set dname
#ncpath <- "/Users/pratik/Desktop/ae_albopictus_model/"   # location of nc file
ncpath <- "/Users/pratik/Desktop/Collab_data/Zia_collab/Pop_count_data_USA/"
ncnamu1 <- "pop_count_" 
ncnamu2 <- "_USA_0.25"
...
for (year in 1995:2019) {  # Replace "year1" and "year2" with your actual year values
  ...
  pop_nc <- paste(ncpath, ncnamu1, year, ncnamu2, ".nc", sep="")
  ...
  pop_month2 = replicate(12,pop_grid_long$pop)  # replicate each year population 12 times in columns to match the format

  # Add the wide_result as additional columns to the final matrix
  pop_month <- cbind(pop_month, as.matrix(pop_month2))  #bind the inital matrix to all matrix
...
## now further combining everything
  ...
  #rm(final_matrix_abun)
  #gc()
  par.a<-2.9        # old estimate of constant a from Sewe estimates
  
  
  w<-par.a* 50      # UPDATED VALUE OF THE CONSTANT FROM REAL DENGUE OUTBREAKS for 10 time step of mosquito abundance
  #w<-par.a* 20      # UPDATED VALUE OF THE CONSTANT FROM REAL DENGUE OUTBREAKS for 100 time step of mosquito abundance
  ...
  ## For any other country or entire globe (like USA or or tropical countries), do this for any other country while using EU data as actual verified data 
  #mean_M = w/z      # here z is the abundance data mean verified with the EU obseravtion and it should be calculated for EU and then constant for any other data areound world
 ...
 # setwd("/Users/pratik/Desktop/ae_albopictus_model")
  ...
  shp_file <- "cb_2020_us_county_500k"
  ...
  req_df <- shp_to_df[,1:9]    # only required information from shapefile not geometry
  ...
  write.csv(final_R0, file = "final_R0_USA1.csv", row.names = FALSE) # set the directory and file name as required
  write.csv(final_LTS, file = "final_LTS_USA1.csv", row.names = FALSE) # set the directory and file name as required
