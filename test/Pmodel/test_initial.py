"""Pytest unit tests for initial.py module functions.
...
from src.model_backend.Pmodel import initial
...
@pytest.fixture
def dummy_rainfall():
    return initial.load_dataset(path_dataset=RAIN_PATH, variable_name="tp")


@pytest.fixture
def dummy_temperature():
    return initial.load_dataset(path_dataset=TEMP_PATH, variable_name="t2m")
...
def test_load_dataset_success():
    ds = initial.load_dataset(path_dataset=RAIN_PATH, variable_name="tp")
...
def test_load_dataset_missing_variable():
    with pytest.raises(KeyError):
        initial.load_dataset(path_dataset=RAIN_PATH, variable_name="not_a_var")


# ---- load_initial_conditions()
def test_load_initial_conditions_default():
    arr = initial.load_initial_conditions(filepath_previous=None, sizes=(5, 5))
    assert isinstance(arr, np.ndarray)
    assert arr.shape == (5, 5, len(initial.MODEL_VARIABLES))
...
def test_load_temperature(dummy_temperature):
    temp, temp_mean = initial.load_temperature(
...
def test_misaligned_coordinates(misaligned_population_nc, misaligned_rainfall_nc):
    pop = initial.load_dataset(
        path_dataset=misaligned_population_nc, variable_name="dens"
    )
    rain = initial.load_dataset(path_dataset=misaligned_rainfall_nc, variable_name="tp")
...
def test_align_population_to_rainfall(misaligned_population_nc, misaligned_rainfall_nc):
    ...
    pop_aligned = initial.align_xarray_datasets(pop, rain)
